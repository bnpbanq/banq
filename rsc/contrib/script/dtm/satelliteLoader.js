(function() {
  ...
  window.launchSatelliteLoader = function (){
    ...
    dtmHost = ( isEnv('PREVIEW') || isEnv('QUALIF') || isEnv('MACHINE') || isMode('dtm-Akamai') || isMode('launch-dev2') || isMode('launch-dev3') || window.ENV_IS_CAS) ? 'Akamai' : 'SelfHosted';

    if (!isEnv("PROD") && !isEnv("PREVIEW") && !isEnv("QUALIF") && !isEnv("MOBILE") && !isEnv("MACHINE")) { return false; } 
    else { script = (isEnv("PROD") || isMode("PROD") || isEnv("MOBILE") || isMode("MOBILE")) ? scripts[dtmHost].PROD : isMode('launch-dev2') ? scripts[dtmHost].DEV2 : isMode('launch-dev3') ? scripts[dtmHost].DEV3 : scripts[dtmHost].PREVIEW; }
